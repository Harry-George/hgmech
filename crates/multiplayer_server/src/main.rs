    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "multiplayer_server=info,tower_http=debug,matchbox_signaling=info".into()),
        )
        .with(
...
async fn create_room_handler(State(state): State<ServerState>) -> Json<RoomId> {
    Json(state.create_room())
}

async fn list_rooms_handler(State(state): State<ServerState>) -> Json<Vec<RoomId>> {
...
mod tests {
    ...
    async fn test_create_and_check_room() {
        ...
        // List rooms
        let response = app.clone()
            .oneshot(Request::builder().method("GET").uri("/rooms").body(Body::empty()).unwrap())
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::OK);
        let body = axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap();
        ...
        // Check room exists
        let response = app.clone()
            .oneshot(Request::builder().method("GET").uri(format!("/rooms/{}", room_id.0)).body(Body::empty()).unwrap())
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::OK);
        ...
        let response = app.clone()
            .oneshot(Request::builder().method("GET").uri("/rooms/non-existent").body(Body::empty()).unwrap())
            .await
            .unwrap();
